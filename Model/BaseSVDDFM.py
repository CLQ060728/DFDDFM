...
class BaseSVDDFM(ABC):
    def __init__(self, device: torch.device, dfm: bool=False, dfm_num_layers: int=3, dfm_num_mani: int=4,
        ...
        if dfm:
            ...
            self.dfm_encoder = SingleManifoldLayer(encoder_decoder_num_layers, hidden_dim)
        ...
        if self.dfm:
            ...
            if self.dfm_aggr == "SUM":
                ...
                for manifold in self.orthogonal_manifolds:
                    ...
                    manifolds_features = torch.cat((manifolds_features, manifold_feature.unsqueeze(0)))  # Concatenate along dimension of manifolds
